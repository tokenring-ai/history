...
export async function execute(remainder, registry) {
 ...
 try {
  ...
  if (selectedSessionId) {
   await displaySessionHistory(sessions.find(({id}) => id === selectedSessionId), historyStorage, chatService);
  } else {
...
async function displaySessionHistory(session, historyStorage, chatService) {
 ...
 chatService.systemLine(`Created: ${new Date(session.createdAt).toLocaleString()}`);
 ...
 try {
  ...
  chatService.errorLine(`Error loading messages for session ${session.id}:`, error);
 }
}
