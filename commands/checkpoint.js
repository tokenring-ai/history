...
export async function execute(remainder, registry) {
 ...
 const checkpointService = registry.requireFirstServiceByType(CheckpointService);
 ...
 switch (action) {
  case "create": {
   if (!currentMessage) {
    chatService.errorLine("No active chat to checkpoint. Ask at least one question first.");
    return;
   }

   const label = args.join(" ") || `New Checkpoint`;
   const row = await checkpointService.createCheckpoint(label, currentMessage);
  ...
  case "restore": {
   if (!args[0]) {
    chatService.errorLine("Usage: /checkpoint restore <id> (see /checkpoint list for ids)");
    return;
   }
   ...
   if (row) {
    const message = await chatMessageStorage.retrieveMessageById(row.messageId);
    if (message) {
     chatService.systemLine(`Checkpoint ${row.id} loaded: ${row.label}`);
     chatMessageStorage.setCurrentMessage(message);
    } else {
     chatService.errorLine(`Message ${row.messageId} not found. Checkpoint ${row.id} exists, but no message loaded.`);
    }
   } else {
  ...
  default: {
   ...
   if (savedCheckpoints.length === 0) {
    chatService.systemLine("No checkpoints saved. Use /checkpoint create to make one.");
    break;
   }
   ...
   try {
    ...
    if (row) {
     ...
     chatService.systemLine("Checkpoint selection cancelled. No changes made.");
    }
   } catch (error) {
