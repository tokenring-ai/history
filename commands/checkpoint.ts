...
export async function execute(
  ...
  switch (action) {
    ...
    default: {
      ...
      const grouped: Record<string, typeof savedCheckpoints> = {} as any;
      for (const cp of savedCheckpoints) {
        const date = new Date(cp.createdAt).toISOString().slice(0, 10);
        if (!grouped[date]) grouped[date] = [] as any;
      ...
      try {
        ...
        const row = await checkpointService.retrieveCheckpoint(selectedId as any);
